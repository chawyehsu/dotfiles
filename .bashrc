...
if [ -x "$(command -v vivid)" ]; then
  _vivid_theme="base16-selenized-black"
  _mode="24-bit"
  _cur_term="${TERM_PROGRAM:-}"
  _termnials=("Apple_Terminal")

  for _term in "${_termnials[@]}"; do
    [ "$_cur_term" = "$_term" ] && _mode="8-bit" && break
  done

  _generated=$(vivid -m $_mode generate $_vivid_theme 2>&1)
