...
if (Test-Command 'vivid') {
    $vividTheme = 'base16-selenized-black'
    if ($env:TERM_PROGRAM -in @('Apple_Terminal')) {
        # for terminals that don't support 24-bit colors
        $vividGenerated = $(vivid -m 8-bit generate $vividTheme 2>&1)
    } else {
        $vividGenerated = $(vivid generate $vividTheme 2>&1)
    }
